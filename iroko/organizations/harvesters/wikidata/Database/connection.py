...
class Connection:
    ...
    def getPool(cls):
        if cls.__pool is None:
            try:
                ...
                print(f'Pool creation successfully: {cls.__pool}')
                return cls.__pool
    ...
    def getConnection(cls):
        ...
        print(f'Connection from pool: {connection}')
        return connection
    ...
    def releaseConnection(cls, connection):
        ...
        print(f'Release connection to pool: {connection}')
        print(f'State of pool: {cls.__pool}')

    @classmethod
    def closeConnections(cls):
        # Cerrar el pool y todas sus conexiones
        cls.getPool().closeall()
        print(f'Close all connections from pool: {cls.__pool}')
