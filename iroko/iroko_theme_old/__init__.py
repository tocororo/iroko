...
:class:`~iroko_theme.ext.InvenioTheme` extension.

>>> from flask import Flask, render_template_string
>>> from iroko_theme import InvenioTheme
...
Invenio instance. Simply set the :data:`iroko_theme.config.THEME_LOGO` and
:data:`iroko_theme.config.THEME_SITENAME` configuration variables:
...
:data:`iroko_theme.config.THEME_SEARCHBAR` to ``False``:
...
:data:`iroko_theme.config.THEME_JAVASCRIPT_TEMPLATE` variable to a template
which includes the tracking code:

>>> app.config.update({
... 'THEME_JAVASCRIPT_TEMPLATE': 'iroko_theme/trackingcode.html',
... })

Note that ``iroko_theme/trackingcode.html`` is empty so you have to provide
...
:data:`iroko_theme.config.THEME_GOOGLE_SITE_VERIFICATION` configuration
...
.. code-block:: jinja
   ...
   {# mytheme/header.html #}
   {% extends "iroko_theme/header.html" %}
...
~~~~~~~~~~~~~~~~~~~~~~~~
The header template (``iroko_theme/header.html``) is reponsible for rendering
...
:data:`iroko_theme.config.THEME_HEADER_TEMPLATE`.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The header login template (``iroko_theme/header_login.html``) is responsible
...
Change the template by updating
:data:`iroko_theme.config.THEME_HEADER_LOGIN_TEMPLATE`

Footer section template
~~~~~~~~~~~~~~~~~~~~~~~
The footer template (``iroko_theme/footer.html``) is primarily reponsible for
rendering the language selector and does not contain any template blocks.

Change the template by updating
:data:`iroko_theme.config.THEME_FOOTER_TEMPLATE`

Javascript section template
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
If all you need to customize is to load some additional JavaScript, you can
just override the JavaScript template (``iroko_theme/javascript.html``). This
template is responsible for including site-wide JavaScript modules.

Change the template by updating
:data:`iroko_theme.config.THEME_JAVASCRIPT_TEMPLATE`

Settings page template
~~~~~~~~~~~~~~~~~~~~~~
The settings template (``iroko_theme/page_settings.html``) is responsible for
rendering user settings pages such as the change password dialog.

.. image:: static/settings.png

Change the template by updating
:data:`iroko_theme.config.SETTINGS_TEMPLATE`
...
Cover page template
~~~~~~~~~~~~~~~~~~~
The cover template (``iroko_theme/page_cover.html``) has a simplified layout
and is used for e.g. displaying the login dialog and sign-up page.

.. image:: static/cover.png

Change the template by updating
:data:`iroko_theme.config.COVER_TEMPLATE`
...
Base page template
~~~~~~~~~~~~~~~~~~
The base template (``iroko_theme/page.html``) is responsible for providing a
...
Change the template by updating
:data:`iroko_theme.config.BASE_TEMPLATE`
...
~~~~~~~~~~~~~~~~~~~~
The error template (``iroko_theme/page_error.html``) is used as basis for
error pages such as unauthorized access, internal server error etc.

.. image:: static/error.png

Change the template by updating
:data:`iroko_theme.config.THEME_ERROR_TEMPLATE`. If you wish to just override
individual error pages you can set the variables:

* :data:`iroko_theme.config.THEME_401_TEMPLATE`
* :data:`iroko_theme.config.THEME_403_TEMPLATE`
* :data:`iroko_theme.config.THEME_404_TEMPLATE`
* :data:`iroko_theme.config.THEME_500_TEMPLATE`

Front page
~~~~~~~~~~
The front page (``iroko_theme/frontpage.html``) is the homepage of the
application. By default is ``disabled``.

Enable the front page view by setting ``THEME_FRONTPAGE`` to ``True``
:data:`iroko_theme.config.THEME_FRONTPAGE`

Change the template by updating:
:data:`iroko_theme.config.THEME_FRONTPAGE_TEMPLATE`
...
Invenio-Theme provides a macro in ``iroko_theme/macros/messages.html`` called
