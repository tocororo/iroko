...
from wtforms.widgets import CheckboxInput, HTMLString, ListWidget, html_params
    ...
    def __call__(self, field, **kwargs):
        ...
        for subfield in field:
            if self.prefix_label:
                html.append(
                    '<li class="class="d-flex align-items-baseline"">%s %s</li>' % (
                    subfield.label, subfield())
                    )
            else:
                html.append(
                    '<li class="d-flex align-items-baseline">%s %s</li>' % (
...
class InclusionForm(FlaskForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        with open(
            current_app.config[
                'INIT_STATIC_JSON_PATH'] + '/' + get_locale() + '/inclusion_requeriments.json'
            ) as file:
